...
function buildReport(
  ...
  const lines = [
    ...
    `- Expectancy (R): ${summary.expectancyR.toFixed(3)}`,
    `- Average PnL: ${summary.avgPnlPct.toFixed(3)}%`,
    `- Max Drawdown: ${summary.maxDrawdownPct.toFixed(2)}%`,
    `- Final Equity: ${summary.finalEquity.toFixed(2)}`,
    '',
    '## Setup Breakdown',
    ...setupBreakdown.map((g) => `- ${g.key}: trades=${g.trades}, winRate=${g.winRate.toFixed(1)}%, avgR=${g.avgR.toFixed(2)}, PF=${g.profitFactor.toFixed(2)}`),
    '',
    '## Regime Breakdown',
    ...regimeBreakdown.map((g) => `- ${g.key}: trades=${g.trades}, winRate=${g.winRate.toFixed(1)}%, avgR=${g.avgR.toFixed(2)}, PF=${g.profitFactor.toFixed(2)}`),
    '',
    '## Exit Breakdown',
    ...exitBreakdown.map((g) => `- ${g.key}: trades=${g.trades}, winRate=${g.winRate.toFixed(1)}%, avgR=${g.avgR.toFixed(2)}, PF=${g.profitFactor.toFixed(2)}`),
    '',
...
function maybeFillPending(pending: PendingSignal | null, k: Kline, index: number, params: BacktestParams): OpenPosition | null {
...
function buildResponse(
  ...
  const reportMarkdown = buildReport(summary, setupBreakdown, regimeBreakdown, exitBreakdown, stateBreakdown, params);
...
function runSnrFvgLqTc(klines: Kline[], params: BacktestParams): BacktestResponse {
  ...
  for (let i = startIndex; i < klines.length; i += 1) {
    ...
    if (!openPosition && pendingSignal) {
      const filled = maybeFillPending(pendingSignal, k, i, params);
      if (filled) {
        bump(`pending_fill_${filled.side}`);
        openPosition = filled;
        pendingSignal = null;
      } else if (i > pendingSignal.expiresAt) {
